  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!hasError) {
      try {
        ...
        if (response.ok) {
          try {
            const roleResponse = await fetch(`${urlBackend}/users/getRoleByUserId/${data.user_id}`, {
              method: 'GET',
              headers: {
                'Content-Type': 'application/json'
              }
            ...
            if (roleResponse) {
              const dataRole = await roleResponse.json();
              sessionStorage.setItem('role', dataRole.role.role_name);
              sessionStorage.setItem('userId', data.user_id);
              if (dataRole.role.role_name === "Administrator") {
                await router.push('/job-management');
              } else if (dataRole.role.role_name === "Assistant") {
                await router.push('/course-management');
              } else if (dataRole.role.role_name === "Job Finder") {
                await router.push('/course');
              } else if (dataRole.role.role_name === "Job Seeker") {
                await router.push('/');
              }
            }
          } catch (error) {
          ...
          if (data.message === 'Wrong password') {
            setError((prevErrors) => ({ ...prevErrors, passwordNotExist: true }));
          } else if (data.message === 'User not found')
            setError((prevErrors) => ({ ...prevErrors, emailNotExist: true }));
        }
      } catch (error) {
                ...
                <div>
                  <div className="flex -mx-2">
                    <div className="w-full px-3 mb-5">
                      ...
                      {error.emailNotExist && <p className="text-xs font-semibold text-red-500">* User not found!</p>}
                    </div>
                  </div>
                  ...
                  <div className="flex -mx-2">
                    <div className="w-full px-3 mb-5">
                      ...
                      {error.emailNotExist && <p className="text-xs font-semibold text-red-500">* Incorrect password!</p>}
                    </div>
                  </div>
                  <div className="flex -mx-2">
                    <div className="w-full px-3 mb-5">
                  ...
                  <div className="flex -mx-2 mb-3">
                    <div className="w-full text-center px-3 mb-5">
                      <label className="text-xs font-semibold px-1">
                        ...
                        <button onClick={handleSignUp} className="py-2 px-5 text-indigo-500 rounded">SIGN UP</button>
                      </label>
                    </div>
                  </div>
                </div>
