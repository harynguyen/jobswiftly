...
export const SignUp: NextPage = () => {
  ...
  const re_password = useRef<null | HTMLInputElement>(null);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const rePasswordValue = re_password.current?.value;
    const termChecked = term.current?.checked;

    // Validation checks
    let hasError = false;
    if (!role) {
      setError((prevErrors) => ({ ...prevErrors, role: true }));
      hasError = true;
    }
    ...
    if (!rePasswordValue || rePasswordValue !== passwordValue) {
      setError((prevErrors) => ({ ...prevErrors, re_password: true }));
      hasError = true;
    }
    if (!termChecked) {
      setError((prevErrors) => ({ ...prevErrors, term: true }));
      hasError = true;
    }
    ...
    if (!hasError) {
      try {
        const response = await fetch(`${urlBackend}/users/createUser`, {
          method: 'POST',
          headers: {
        ...
        if (response.ok) {
          ...
          setError((prevErrors) => ({ ...prevErrors, emailExist: true }));
        }
      } catch (error) {
      ...
      <div className="min-w-screen min-h-screen bg-gray-900 flex items-center justify-center px-5 py-5">
        <div className="bg-gray-100 text-gray-500 rounded-3xl shadow-xl w-full overflow-hidden" style={{ maxWidth: "1000px" }}>
          <div className="md:flex w-full">
            <div className="hidden justify-center items-center md:block w-1/2 bg-indigo-500 py-20 px-10">
                ...
                <div>
                  <div className="flex -mx-2">
                    <div className="w-full px-3 mb-5">
                      <label htmlFor="role" className="text-xs font-semibold px-1">Select your account role:</label>
                      <select
                        id="role"
                      ...
                      >
                        <option disabled value="">Select the role of the account</option>
                        <option value="Job Seeker">Job Seeker</option>
                        <option value="Job Finder">Job Finder</option>
                      </select>
                      {error.role && <p className="text-red-500 text-xs mt-1">Role is required</p>}
                    </div>
                  </div>
                  <div className="flex -mx-2">
                    <div className="w-full px-3 mb-5">
                      ...
                      {error.email && <p className="text-xs font-semibold text-red-500">* Please enter a valid email address!</p>}
                      {error.emailExist && <p className="text-xs font-semibold text-red-500">* This email already exist!</p>}
                    </div>
                  </div>
                  ...
                  <div className="flex -mx-2">
                    <div className="w-full px-3 mb-5">
                      ...
                      {error.password && <p className="text-xs font-semibold text-red-500">* Please enter your password!</p>}
                    </div>
                  </div>
                  <div className="flex -mx-2">
                    <div className="w-full px-3 mb-5">
                      {/* Display password strength feedback */}
                      <p className="text-xs font-semibold px-1">Password Strength: {password.current && zxcvbn(password.current.value).score + 1}/5</p>
                    </div>
                  </div>
                  <div className="flex -mx-2">
                    <div className="w-full px-3 mb-5">
                      ...
                      {error.re_password && <p className="text-xs font-semibold text-red-500">* Please re-enter your password!</p>}
                    </div>
                  </div>
                  <div className="flex -mx-2 mb-5">
                    <div className="w-full px-3">
                      <label className="font-semibold px-1">
                        ...
                        <a href="/TermsNConditions" className="py-2 hover:text-indigo-700 focus:text-indigo-700 text-indigo-500 rounded" target="_blank" rel="noopener noreferrer">
                          &nbsp;Terms and Conditions
                        </a>
                      </label>
                      {error.term && <p className="text-red-500 text-xs font-semibold">* Please agree to the terms and conditions!</p>}
                    </div>
                  </div>
                  ...
                  <div className="flex -mx-2 mb-3">
                    <div className="w-full text-center px-3 mb-5">
                      <label className="text-xs font-semibold px-1">
                        Already have an account?
                        <button onClick={handleSignIn} className="py-2 px-5 text-indigo-500 rounded">SIGN IN</button>
                      </label>
                    </div>
                  </div>
                </div>
