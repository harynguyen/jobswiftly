...
interface FormData {
    ...
    job_requirement_tech: { value: string; label: string }[] | null;
    job_location: { value: string; label: string }[] | null;
...
const JobCreateForm: React.FC<JobCreateFormProps> = ({ refresh }) => {
    const [formData, setFormData] = useState<FormData>({
        job_name: "",
        job_number_require: 0,
        job_description: "",
        job_requirement_tech: [],
        job_location: [],
        job_salary_range: "",
        job_level: "",
        job_type: "",
    ...
    const [selectedOptionTechnologies, setSelectedOptionTechnologies] = useState<{ value: string; label: string }[] | null>(null);
    const [selectedOptionLocations, setSelectedOptionLocations] = useState<{ value: string; label: string }[] | null>(null);
    ...
    const handleJobSubmission = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const data: FormData = {
            ...
            job_number_require: formData.job_number_require || 0,
            job_description: formData.job_description || "",
            job_requirement_tech: selectedOptionTechnologies ? selectedOptionTechnologies : [],
            job_location: selectedOptionLocations ? selectedOptionLocations : [],
        ...
        try {
            const response = await fetch(`${urlBackend}/job/createJob`, {
                method: "POST",
            ...
            if (response.ok) {
                window.alert("Post New Job Successfully!!!");
                refresh();
                resetFormData();
            }
        } catch (error) {
    ...
    const resetFormData = () => {
        ...
        setSelectedOptionTechnologies(null);
        setSelectedOptionLocations(null);
    };
    ...
    return (
        <form onSubmit={handleJobSubmission} className='p-2 m-2'>
            ...
            <div className='create-job-flex m-1 p-1'>
                <div className='lg:w-4/12 w-full'>
                    <label className='block font-semibold'>Company of Job:</label>
                    <select
                        className='create-job-input pl-10 pr-3 py-2 rounded-lg ring-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-500'
            ...
            <div className='create-job-flex m-1 p-1'>
                <div className='w-full'>
                    <label className='block font-semibold'>Description of Job:</label>
                    <textarea className='create-job-input pl-10 pr-3 py-2 rounded-lg ring-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-500' rows={6} placeholder='Describe the job here' value={formData.job_description} onChange={(e) => setFormData({ ...formData, job_description: e.target.value })} />
            ...
            <div className='w-full m-1 p-1'>
                <div className="w-full ">
                    <button className="block w-full max-w-xs mx-auto bg-indigo-500 hover:bg-indigo-700 focus:bg-indigo-700 text-white rounded-lg px-3 py-3 font-semibold">Post Job</button>
                </div>
            </div>
        </form>
    );
